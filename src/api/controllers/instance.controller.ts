...
export class InstanceController {
  ...
  public async createInstance({
    ...
    try {
      this.logger.verbose(
        'requested createInstance from ' + instanceName + ' instance',
      )
        ...
        try {
          if (!isURL(typebot_url, {require_tld: false})) {
            throw new BadRequestException(
              'Invalid "url" property in typebot_url',
            )
          }
      ...
      try {
        this.chatwootService.create(instance, {
          ...
          name_inbox:
            chatwoot_name_inbox ?? instance.instanceName.split('-cwId-')[0],
          number,
  ...
  public async restartInstance({instanceName}: InstanceDto) {
    try {
      this.logger.verbose(
        'requested restartInstance from ' + instanceName + ' instance',
      )
  ...
  public async registerMobileCode(
    {instanceName}: InstanceDto,
    {mobileCode}: any,
  ) {
    try {
      this.logger.verbose(
        'requested registerMobileCode from ' + instanceName + ' instance',
      )
      ...
      return {
        status: 'SUCCESS',
        error: false,
        response: {message: 'Mobile code registered'},
      }
    } catch (error) {
      this.logger.error(error)
    }
  ...
  public async connectionState({instanceName}: InstanceDto) {
    this.logger.verbose(
      'requested connectionState from ' + instanceName + ' instance',
    )
    return {
      instance: {
        instanceName: instanceName,
  ...
  public async setPresence({instanceName}: InstanceDto, data: SetPresenceDto) {
    this.logger.verbose(
      'requested sendPresence from ' + instanceName + ' instance',
    )
  ...
  public async deleteInstance({instanceName}: InstanceDto) {
    this.logger.verbose(
      'requested deleteInstance from ' + instanceName + ' instance',
    )
    const {instance} = await this.connectionState({instanceName})
    ...
    try {
      ...
      try {
        this.waMonitor.waInstances[instanceName]?.sendDataWebhook(
          Events.INSTANCE_DELETE,
          {
            instanceName,
            instanceId: (await this.repository.auth.find(instanceName))
              ?.instanceId,
          },
        )
      } catch (error) {
