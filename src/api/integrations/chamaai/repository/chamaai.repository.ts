...
export class ChamaaiRepository extends Repository {
  constructor(
    private readonly chamaaiModel: IChamaaiModel,
  ...
  public async create(data: ChamaaiRaw, instance: string): Promise<IInsert> {
    try {
      ...
      if (this.dbSettings.ENABLED) {
        ...
        const insert = await this.chamaaiModel.replaceOne(
          {_id: instance},
          {...data},
          {upsert: true},
        )
        ...
        this.logger.verbose(
          'chamaai saved to db: ' + insert.modifiedCount + ' chamaai',
        )
      ...
      this.logger.verbose(
        'chamaai saved to store in path: ' +
          join(this.storePath, 'chamaai') +
          '/' +
          instance,
      )
