...
export class ChamaaiService {
  ...
  private getRegexPatterns() {
    ...
    const regexPatterns = patternsToCheck.map(
  ...
  public async sendChamaai(instance: InstanceDto, remoteJid: string, msg: any) {
    ...
    if (msgType === 'audioMessage') {
      const downloadBase64 = await this.waMonitor.waInstances[
        instance.instanceName
    ...
    if (answer) {
      ...
      if (type === 'audio') {
        this.waMonitor.waInstances[instance.instanceName].audioWhatsapp({
          number: remoteJid.split('@')[0],
          options: {
            delay: characterCount
              ? this.convertToMilliseconds(characterCount) * 1000 || 1000
              : 1000,
            presence: 'recording',
            encoding: true,
          },
