...
export class BaileysStartupService extends ChannelStartupService {
      ...
      try {
        for (const received of messages) {
          ...
          if (
            ...
            this.logger.info(
              `Recovering message lost messageId: ${received.key.id}`,
            )
          ...
          if (
            ...
            if (received.key.fromMe === false) {
              if (msg.status === status[3]) {
                this.logger.log(
                  `Update not read messages ${received.key.remoteJid}`,
                )

                await this.updateChatUnreadMessages(received.key.remoteJid)
              } else if (msg.status === status[4]) {
                this.logger.log(
                  `Update readed messages ${received.key.remoteJid} - ${msg.messageTimestamp}`,
                )
          ...
          if (this.localWebhook.enabled) {
            if (isMedia && this.localWebhook.webhookBase64) {
              try {
                ...
                messageRaw.message.base64 = buffer
          ...
          if (update.message === null && update.status === undefined) {
            ...
            if (!key.fromMe && key.remoteJid) {
              ...
              if (status[update.status] === status[4]) {
                this.logger.log(
                  `Update as read ${key.remoteJid} - ${findMessage.messageTimestamp}`,
                )
  ...
  private async sendMessage(
    ...
    if (messageId) option.messageId = messageId
      ...
      option.messageId = '3EB0' + randomBytes(18).toString('hex').toUpperCase()
  ...
  public async processAudio(audio: string): Promise<Buffer> {
    if (process.env.API_AUDIO_CONVERTER) {
      ...
      const {data} = await axios.post(
        process.env.API_AUDIO_CONVERTER,
        formData,
        {
          headers: {
            ...formData.getHeaders(),
      ...
      jids.users.map(async (user) => {
        ...
        if (cached) {
          return {
            ...
            name: contacts.find((c) => c.remoteJid === cached.remoteJid)
              ?.pushName,
            number: user.number,
          }
        }
