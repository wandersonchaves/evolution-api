  ...
  implements ChatbotControllerInterface
{
  ...
  public async createBot(instance: InstanceDto, data: FlowiseDto) {
    ...
    if (checkTriggerAll && data.triggerType === 'all') {
      throw new Error(
        'You already have a Flowise with an "All" trigger, you cannot have more bots while it is active',
      )
    }
  ...
  public async updateBot(
    ...
    if (data.triggerType === 'all') {
      ...
      if (checkTriggerAll) {
        throw new Error(
          'You already have a bot with an "All" trigger, you cannot have more bots while it is active',
        )
      }
    }
