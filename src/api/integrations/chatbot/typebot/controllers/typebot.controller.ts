  ...
  public async createBot(instance: InstanceDto, data: TypebotDto) {
    ...
    if (
      ...
      if (!data.keywordFinish)
        data.keywordFinish = defaultSettingCheck?.keywordFinish || '#SAIR'
      if (!data.delayMessage)
        data.delayMessage = defaultSettingCheck?.delayMessage || 1000
      if (!data.unknownMessage)
        data.unknownMessage =
          defaultSettingCheck?.unknownMessage || 'Desculpe, não entendi'
      if (!data.listeningFromMe)
        data.listeningFromMe = defaultSettingCheck?.listeningFromMe || false
      if (!data.stopBotFromMe)
        data.stopBotFromMe = defaultSettingCheck?.stopBotFromMe || false
      if (!data.keepOpen) data.keepOpen = defaultSettingCheck?.keepOpen || false
      if (!data.debounceTime)
        data.debounceTime = defaultSettingCheck?.debounceTime || 0
      if (!data.ignoreJids)
    ...
    try {
      ...
      if (status === 'delete') {
        ...
        return {
          typebot: {
            ...instance,
            typebot: {remoteJid: remoteJid, status: status},
          },
        }
      }
