...
export class ChatwootService {
  ...
  private findContactInContactList(contacts: any[], query: string) {
    ...
    if (
      contacts.length === 2 &&
      this.getClientCwConfig().merge_brazil_contacts &&
      query.startsWith('+55')
    ) {
  ...
  public async createConversation(instance: InstanceDto, body: any) {
    this.logger.verbose(
      'create conversation to instance: ' + instance.instanceName,
    )
    try {
      const client = await this.clientCw(instance)
  ...
  public async getInbox(instance: InstanceDto) {
    ...
    let findByName = inbox.payload.find(
      (inbox) => inbox.name === this.getClientCwConfig().name_inbox,
    )

    if (!findByName) {
      findByName = inbox.payload.find(
        (inbox) =>
          inbox.name === this.getClientCwConfig().name_inbox.split('-cwId-')[0],
      )
    }
  ...
  public async createBotMessage(
    ...
    this.logger.verbose(
      'create bot message to instance: ' + instance.instanceName,
    )
  ...
  public async sendAttachment(
    ...
    this.logger.verbose(
      'send attachment to instance: ' + waInstance.instanceName,
    )
    ...
    try {
      ...
      if (type === 'audio') {
        this.logger.verbose(
          'send audio to instance: ' + waInstance.instanceName,
        )
    ...
    try {
      ...
      if (
        ...
        if (message.length && message[0].key?.id) {
          this.logger.verbose(
            'deleting message in whatsapp. Message id: ' + message[0].key.id,
          )
          await waInstance?.client.sendMessage(message[0].key.remoteJid, {
            delete: message[0].key,
          })

          this.logger.verbose(
            'deleting message in repository. Message id: ' + message[0].key.id,
          )
      ...
      if (chatId === '123456' && body.message_type === 'outgoing') {
        ...
        if (command.includes('init') || command.includes('iniciar')) {
          ...
          if (state !== 'open') {
            if (state === 'close') {
              this.logger.verbose(
                'request cleaning up instance: ' + instance.instanceName,
              )
            }
      ...
      if (
        ...
        if (
          ...
          this.logger.verbose(
            'message sent directly from whatsapp. Webhook ignored.',
          )
          return {message: 'bot'}
        }
        ...
        if (senderName === null || senderName === undefined) {
          ...
          const formattedDelimiter = this.provider.sign_delimiter
            ...
            : '\n'
          const textToConcat = this.provider.sign_msg
            ? [`*${senderName}:*`]
            : []
  ...
  private getReactionMessage(msg: any) {
    ...
    reactionMessage &&
      this.logger.verbose('Reaction message: ' + reactionMessage)
  ...
  private getMessageContent(types: any) {
    ...
    if (typeKey === 'locationMessage' || typeKey === 'liveLocationMessage') {
      ...
      const formattedLocation =
        ...
        (locationName
          ? `_${i18next.t(
              'cw.locationMessage.locationName',
            )}:_ ${locationName}\n`
          : '') +
        (locationAddress
          ? `_${i18next.t(
              'cw.locationMessage.locationAddress',
            )}:_ ${locationAddress} \n`
          : '') +
    ...
    if (typeKey === 'contactMessage') {
      ...
      Object.keys(contactInfo).forEach((key) => {
        if (key.startsWith('item') && key.includes('TEL')) {
          ...
          formattedContact += `\n_${i18next.t(
            'cw.contactMessage.number',
          )} (${numberCount}):_ ${phoneNumber}`
          numberCount++
    ...
    if (typeKey === 'contactsArrayMessage') {
      const formattedContacts = result.contacts.map((contact) => {
        ...
        let formattedContact = `*${i18next.t(
          'cw.contactMessage.contact',
  ...
  public async eventWhatsapp(event: string, instance: InstanceDto, body: any) {
    ...
    try {
      ...
      if (event === 'messages.upsert' || event === 'send.message') {
        ...
        if (
          bodyMessage &&
          bodyMessage.includes('Por favor, classifique esta conversa, http')
        ) {
        ...
        if (isMedia) {
          ...
          if (originalFilename) {
            ...
            if (parsedFile.name && parsedFile.ext) {
              nameFile = `${parsedFile.name}-${Math.floor(
      ...
      if (event === Events.MESSAGES_DELETE) {
        ...
        if (chatwootDelete === true) {
          this.logger.verbose(
            'deleting message from instance: ' + instance.instanceName,
          )
          ...
          if (
            message?.chatwoot?.messageId &&
            message?.chatwoot?.conversationId
          ) {
            this.logger.verbose(
              'deleting message in repository. Message id: ' + body.key.id,
            )
            this.repository.message.delete({
              where: {
                key: {
            ...
            this.logger.verbose(
              'deleting message in chatwoot. Message id: ' + body.key.id,
            )
      ...
      if (event === 'messages.edit') {
        ...
        if (message && message.chatwoot?.conversationId) {
          const send = await this.createMessage(
            ...
            {
              message: {
                extendedTextMessage: {contextInfo: {stanzaId: message.key.id}},
              },
            },
            'WAID:' + body.key.id,
          )
      ...
      if (event === 'messages.read') {
        this.logger.verbose(
          'read message from instance: ' + instance.instanceName,
        )
  ...
  public async updateContactAvatarInRecentConversations(
    ...
    try {
      ...
      const contactsWithProfilePicture = (
        ...
        (acc: Map<string, ContactRaw>, contact: ContactRaw) =>
