...
class ChatwootImport {
  ...
  public addHistoryMessages(instance: InstanceDto, messagesRaw: MessageRaw[]) {
    ...
    this.historyMessages.set(
      instance.instanceName,
      actualValue.concat(messagesRaw),
    )
  }
  ...
  public async importHistoryMessages(
    ...
    try {
      ...
      allMessagesMappedByPhoneNumber.forEach(
        (messages: MessageRaw[], phoneNumber: string) => {
      ...
      let messagesChunk: MessageRaw[] = this.sliceIntoChunks(
        messagesOrdered,
        batchSize,
      )
        ...
        if (messagesByPhoneNumber.size > 0) {
          ...
          if (bindInsertMsg.length > 2) {
            ...
            totalMessagesImported +=
              (await pgClient.query(sqlInsertMsg, bindInsertMsg))?.rowCount ?? 0
          }
        }
  ...
  public async selectOrCreateFksFromChatwoot(
    ...
    return new Map(
      fksFromChatwoot.rows.map((item: FksChatwoot) => [
        item.phone_number,
        item,
      ]),
    )
  ...
  public createMessagesMapByPhoneNumber(
    messages: MessageRaw[],
  ): Map<string, MessageRaw[]> {
    return messages.reduce(
      (acc: Map<string, MessageRaw[]>, message: MessageRaw) => {
  ...
  public async getContactsOrderByRecentConversations(
    ...
    try {
      ...
      return (await pgClient.query(sql, [provider.account_id, inbox.id, limit]))
        ?.rows
    } catch (error) {
      this.logger.error(
