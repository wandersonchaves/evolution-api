  ...
  implements EventControllerInterface
{
  ...
  constructor(
    ...
    super(
      prismaRepository,
      waMonitor,
      configService.get<Websocket>('WEBSOCKET')?.ENABLED,
      'websocket',
    )
  ...
  public init(httpServer: Server): void {
    ...
    this.socket.on('connection', (socket) => {
      ...
      socket.on('sendNode', async (data) => {
        try {
          await this.waMonitor.waInstances[data.instanceId].baileysSendNode(
            data.stanza,
          )
  ...
  public async emit({
    ...
    try {
      ...
      if (
        Array.isArray(instance?.events) &&
        instance?.events.includes(configEv)
      ) {
