...
export class RabbitmqRepository extends Repository {
  constructor(
    private readonly rabbitmqModel: IRabbitmqModel,
  ...
  public async create(data: RabbitmqRaw, instance: string): Promise<IInsert> {
    try {
      ...
      if (this.dbSettings.ENABLED) {
        ...
        const insert = await this.rabbitmqModel.replaceOne(
          {_id: instance},
          {...data},
          {upsert: true},
        )
        ...
        this.logger.verbose(
          'rabbitmq saved to db: ' + insert.modifiedCount + ' rabbitmq',
        )
      ...
      this.logger.verbose(
        'rabbitmq saved to store in path: ' +
          join(this.storePath, 'rabbitmq') +
          '/' +
          instance,
      )
