...
export class TypebotRepository extends Repository {
  constructor(
    private readonly typebotModel: ITypebotModel,
  ...
  public async create(data: TypebotRaw, instance: string): Promise<IInsert> {
    try {
      ...
      if (this.dbSettings.ENABLED) {
        ...
        const insert = await this.typebotModel.replaceOne(
          {_id: instance},
          {...data},
          {upsert: true},
        )
        ...
        this.logger.verbose(
          'typebot saved to db: ' + insert.modifiedCount + ' typebot',
        )
      ...
      this.logger.verbose(
        'typebot saved to store in path: ' +
          join(this.storePath, 'typebot') +
          '/' +
          instance,
      )
