...
export class TypebotService {
  ...
  public async changeStatus(instance: InstanceDto, data: any) {
    ...
    const session = findData.sessions.find(
      (session) => session.remoteJid === remoteJid,
    )
  ...
  public async clearSessions(instance: InstanceDto, remoteJid: string) {
    ...
    const sessionWithRemoteJid = sessions.filter(
      (session) => session.remoteJid === remoteJid,
    )
  ...
  private getListResponseMessageContent(msg: any) {
    ...
    this.logger.verbose(
      'listResponseMessage selectedRowId: ' + listResponseContent,
    )
  ...
  public async sendWAMessage(
    ...
    function applyFormatting(element) {
      ...
      let formattedText = `${formats}${text}${formats
        .split('')
        .reverse()
        .join('')}`
  ...
  public async sendTypebot(
    ...
    try {
      if (session && expire && expire > 0) {
        ...
        if (diffInMinutes > expire) {
          ...
          if (data.messages.length === 0) {
            ...
            if (
              keyword_finish &&
              content.toLowerCase() === keyword_finish.toLowerCase()
            ) {
              const newSessions = await this.clearSessions(instance, remoteJid)
      ...
      let reqData: {}
      if (version === 'latest') {
        urlTypebot = `${url}/api/v1/sessions/${
