...
export class WebsocketRepository extends Repository {
  constructor(
    private readonly websocketModel: IWebsocketModel,
  ...
  public async create(data: WebsocketRaw, instance: string): Promise<IInsert> {
    try {
      ...
      if (this.dbSettings.ENABLED) {
        ...
        const insert = await this.websocketModel.replaceOne(
          {_id: instance},
          {...data},
          {upsert: true},
        )
        ...
        this.logger.verbose(
          'websocket saved to db: ' + insert.modifiedCount + ' websocket',
        )
      ...
      this.logger.verbose(
        'websocket saved to store in path: ' +
          join(this.storePath, 'websocket') +
          '/' +
          instance,
      )
