...
export class AuthRepository extends Repository {
  ...
  public async create(data: AuthRaw, instance: string): Promise<IInsert> {
    try {
      ...
      if (this.dbSettings.ENABLED) {
        ...
        this.logger.verbose(
          'auth saved to db: ' + insert.modifiedCount + ' auth',
        )
      ...
      this.logger.verbose(
        'auth saved to store in path: ' +
          join(AUTH_DIR, this.auth.TYPE) +
          '/' +
          instance,
      )
  ...
  public async findInstanceNameById(
    instanceId: string,
  ): Promise<string | null> {
    try {
  ...
  public async findInstanceNameByNumber(
    number: string,
  ): Promise<string | null> {
    try {
