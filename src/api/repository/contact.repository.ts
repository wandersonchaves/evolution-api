  ...
  public async insert(
    ...
    try {
      if (this.dbSettings.ENABLED && saveDb) {
        ...
        this.logger.verbose(
          'contacts saved to db: ' + insert.length + ' contacts',
        )
      ...
      if (store.CONTACTS) {
        ...
        this.logger.verbose(
          'contacts saved to store: ' + data.length + ' contacts',
        )
  ...
  public async update(
    ...
    try {
      ...
      if (this.dbSettings.ENABLED && saveDb) {
        ...
        this.logger.verbose(
          'contacts updated in db: ' + nModified + ' contacts',
        )
      ...
      if (store.CONTACTS) {
        ...
        this.logger.verbose(
          'contacts updated in store: ' + data.length + ' contacts',
        )
    ...
    try {
      ...
      if (query?.where?.id) {
        ...
        const openDir = opendirSync(
          join(this.storePath, 'contacts', query.where.owner),
          {
            encoding: 'utf-8',
          },
        )
      ...
      this.logger.verbose(
        'contacts found in store: ' + contacts.length + ' contacts',
      )
      return contacts
    } catch (error) {
      return []
      ...
      if (query.ids.length > 0) {
        ...
        query.ids.forEach((id) => {
          contacts.push(
            JSON.parse(
              readFileSync(
                join(this.storePath, 'contacts', query.owner, id + '.json'),
                {
                  encoding: 'utf-8',
                },
              ),
        ...
        const openDir = opendirSync(
          join(this.storePath, 'contacts', query.owner),
          {
            encoding: 'utf-8',
          },
        )
        for await (const dirent of openDir) {
          if (dirent.isFile()) {
            contacts.push(
              JSON.parse(
                readFileSync(
                  join(this.storePath, 'contacts', query.owner, dirent.name),
                  {
                    encoding: 'utf-8',
                  },
                ),
