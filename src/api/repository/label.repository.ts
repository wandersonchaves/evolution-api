...
export class LabelRepository extends Repository {
  constructor(
    private readonly labelModel: ILabelModel,
  ...
  public async insert(
    ...
    try {
      if (this.dbSettings.ENABLED && saveDb) {
        ...
        const insert = await this.labelModel.findOneAndUpdate(
          {id: data.id},
          data,
          {upsert: true},
        )
      ...
      if (store.LABELS) {
        ...
        this.logger.verbose(
          'labels saved to store in path: ' +
            join(this.storePath, 'labels', instanceName) +
            '/' +
            data.id,
        )
  ...
  public async find(query: LabelQuery): Promise<LabelRaw[]> {
    try {
      ...
      const openDir = opendirSync(
        join(this.storePath, 'labels', query.where.owner),
      )
      for await (const dirent of openDir) {
        if (dirent.isFile()) {
          labels.push(
            JSON.parse(
              readFileSync(
                join(this.storePath, 'labels', query.where.owner, dirent.name),
                {
                  encoding: 'utf-8',
                },
              ),
