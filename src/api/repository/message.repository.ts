  ...
  public async insert(
    ...
    try {
      if (this.dbSettings.ENABLED && saveDb) {
        ...
        const cleanedData = data.map((obj) => {
          ...
          if ('extendedTextMessage' in obj.message) {
            ...
            if (
              typeof extendedTextMessage === 'object' &&
        ...
        this.logger.verbose(
          'messages saved to db: ' + insert.length + ' messages',
        )
      ...
      if (store.MESSAGES) {
        ...
        this.logger.verbose(
          'messages saved to store: ' + data.length + ' messages',
        )
    ...
    try {
      ...
      if (query?.where?.key?.id) {
        ...
        const openDir = opendirSync(
          join(this.storePath, 'messages', query.where.owner),
          {
            encoding: 'utf-8',
          },
        )
      ...
      this.logger.verbose(
        'messages found in store: ' + messages.length + ' messages',
      )
      return messages
        .sort((x, y) => {
  ...
  public async update(
    ...
    try {
      if (this.dbSettings.ENABLED && saveDb) {
        ...
        this.logger.verbose(
          'messages updated in db: ' + nModified + ' messages',
        )
      ...
      if (store.MESSAGES) {
        ...
        this.logger.verbose(
          'messages updated in store: ' + data.length + ' messages',
        )
        return {insertCount: data.length}
      }
