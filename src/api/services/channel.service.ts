...
import {
  getSQS,
  removeQueues as removeQueuesSQS,
} from '../integrations/sqs/libs/sqs.server'
...
export class ChannelStartupService {
  ...
  public set instanceName(name: string) {
    ...
    if (!name) {
      this.logger.verbose(
        'Instance name not found, generating random name with uuid',
      )
  ...
  public async findIntegration() {
    ...
    if (!data) {
      this.repository.integration.create(
  ...
  public async loadSettings() {
    ...
    this.logger.verbose(
      `Settings reject_call: ${this.localSettings.reject_call}`,
    )
    ...
    this.logger.verbose(
      `Settings groups_ignore: ${this.localSettings.groups_ignore}`,
    )

    this.localSettings.always_online = data?.always_online
    this.logger.verbose(
      `Settings always_online: ${this.localSettings.always_online}`,
    )

    this.localSettings.read_messages = data?.read_messages
    this.logger.verbose(
      `Settings read_messages: ${this.localSettings.read_messages}`,
    )

    this.localSettings.read_status = data?.read_status
    this.logger.verbose(
      `Settings read_status: ${this.localSettings.read_status}`,
    )

    this.localSettings.sync_full_history = data?.sync_full_history
    this.logger.verbose(
      `Settings sync_full_history: ${this.localSettings.sync_full_history}`,
    )
  ...
  public async loadWebhook() {
    ...
    this.logger.verbose(
      `Webhook by events: ${this.localWebhook.webhook_by_events}`,
    )

    this.localWebhook.webhook_base64 = data?.webhook_base64
    this.logger.verbose(
      `Webhook by webhook_base64: ${this.localWebhook.webhook_base64}`,
    )
  ...
  public async loadChatwoot() {
    ...
    this.logger.verbose(
      `Chatwoot merge brazil contacts: ${this.localChatwoot.merge_brazil_contacts}`,
    )

    this.localChatwoot.import_contacts = data?.import_contacts
    this.logger.verbose(
      `Chatwoot import contacts: ${this.localChatwoot.import_contacts}`,
    )

    this.localChatwoot.import_messages = data?.import_messages
    this.logger.verbose(
      `Chatwoot import messages: ${this.localChatwoot.import_messages}`,
    )

    this.localChatwoot.days_limit_import_messages =
      data?.days_limit_import_messages
    this.logger.verbose(
      `Chatwoot days limit import messages: ${this.localChatwoot.days_limit_import_messages}`,
    )
  ...
  public async setChatwoot(data: ChatwootRaw) {
    ...
    this.logger.verbose(
      `Chatwoot reopen conversation: ${data.reopen_conversation}`,
    )
    this.logger.verbose(
      `Chatwoot conversation pending: ${data.conversation_pending}`,
    )
    this.logger.verbose(
      `Chatwoot merge brazil contacts: ${data.merge_brazil_contacts}`,
    )
    ...
    this.logger.verbose(
      `Chatwoot days limit import messages: ${data.days_limit_import_messages}`,
    )

    Object.assign(this.localChatwoot, {
      ...data,
      sign_delimiter: data.sign_msg ? data.sign_delimiter : null,
    })
  ...
  public async findChatwoot() {
    ...
    this.logger.verbose(
      `Chatwoot merge brazilian contacts: ${data.merge_brazil_contacts}`,
    )
    this.logger.verbose(`Chatwoot import contacts: ${data.import_contacts}`)
    this.logger.verbose(`Chatwoot import messages: ${data.import_messages}`)
    this.logger.verbose(
  ...
  public async loadTypebot() {
    ...
    this.logger.verbose(
      `Typebot keyword_finish: ${this.localTypebot.keyword_finish}`,
    )

    this.localTypebot.delay_message = data?.delay_message
    this.logger.verbose(
      `Typebot delay_message: ${this.localTypebot.delay_message}`,
    )

    this.localTypebot.unknown_message = data?.unknown_message
    this.logger.verbose(
      `Typebot unknown_message: ${this.localTypebot.unknown_message}`,
    )

    this.localTypebot.listening_from_me = data?.listening_from_me
    this.logger.verbose(
      `Typebot listening_from_me: ${this.localTypebot.listening_from_me}`,
    )
  ...
  public async loadChamaai() {
    ...
    this.logger.verbose(
      `Chamaai answerByAudio: ${this.localChamaai.answerByAudio}`,
    )
  ...
  public async sendDataWebhook<T = any>(event: Events, data: T, local = true) {
    ...
    const rabbitmqGlobal =
      this.configService.get<Rabbitmq>('RABBITMQ').GLOBAL_ENABLED
        ...
        if (Array.isArray(rabbitmqLocal) && rabbitmqLocal.includes(we)) {
          ...
          while (retry < 3) {
            try {
              ...
              if (
                this.configService.get<Log>('LOG').LEVEL.includes('WEBHOOKS')
              ) {
                const logData = {
    ...
    if (this.configService.get<Websocket>('WEBSOCKET')?.ENABLED) {
      this.logger.verbose(
        'Sending data to websocket on channel: ' + this.instance.name,
      )
      const io = getIO()
      ...
      if (
        this.localWebsocket.enabled &&
        Array.isArray(websocketLocal) &&
        websocketLocal.includes(we)
      ) {
        this.logger.verbose('Sending data to websocket on event: ' + event)

        this.logger.verbose(
          'Sending data to socket.io in channel: ' + this.instance.name,
        )
      ...
      if (Array.isArray(webhookLocal) && webhookLocal.includes(we)) {
        ...
        try {
          if (
            ...
            isURL(this.localWebhook.url, {require_tld: false})
          ) {
  ...
  public createJid(number: string): string {
    ...
    if (
      ...
      this.logger.verbose(
        'Number already contains @g.us or @s.whatsapp.net or @lid',
      )
      return number
    }
    ...
    this.logger.verbose(
      'Jid created is whatsapp: ' + `${number}@s.whatsapp.net`,
    )
