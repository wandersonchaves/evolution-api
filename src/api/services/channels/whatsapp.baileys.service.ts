...
export class BaileysStartupService extends ChannelStartupService {
        ...
        this.baileysCache.keys().then((keys) => {
          keys.forEach(async (key) => {
            ...
            if (
              ...
              this.logger.info(
                'Message absent from node, retrying to send, key: ' +
                  key.split(':')[2],
              )
              await this.client.sendMessageAck(
                JSON.parse(
                  message.messageStubParameters[1],
                  BufferJSON.reviver,
                ),
              )
            }
    ...
    if (!profileName) {
      ...
      if (this.configService.get<Database>('DATABASE').ENABLED) {
        ...
        const collection = dbserver
          .getClient()
          .db(
            this.configService.get<Database>('DATABASE').CONNECTION
              .DB_PREFIX_NAME + '-instances',
          )
        ...
        existsSync(join(INSTANCE_DIR, this.instanceName, 'creds.json'))
      ) {
  ...
  private async connectionUpdate({
    ...
    if (qr) {
      ...
      if (
        ...
        this.logger.verbose(
          'Sending data to webhook in event CONNECTION_UPDATE',
        )
  ...
  private async createClient(
    ...
    const socketConfig: UserFacingSocketConfig = {
      ...
      shouldIgnoreJid: (jid) => {
        ...
        const isBroadcast =
          !this.localSettings.read_status && isJidBroadcast(jid)
  ...
  private readonly contactHandle = {
    'contacts.upsert': async (contacts: Contact[], database: Database) => {
      try {
        ...
        this.logger.verbose(
          'Verifying if contacts exists in database to insert',
        )
        ...
        this.repository.contact.insert(
          contactsRaw,
          this.instance.name,
        ...
        if (
          ...
          this.localChatwoot.import_contacts &&
          contactsRaw.length
        ) {
        ...
        for await (const contact of contacts) {
          contactsRaw.push({
            ...
            pushName:
              ...
              contact?.verifiedName ||
              contact.id.split('@')[0],
            profilePictureUrl: (await this.profilePicture(contact.id))
              .profilePictureUrl,
            owner: this.instance.name,
          })
        }
    ...
    'messaging-history.set': async (
      ...
      try {
        ...
        const daysLimitToImport = this.localChatwoot.enabled
          ? this.localChatwoot.days_limit_import_messages
          : 1000
        this.logger.verbose(
          `Param days limit import messages is: ${daysLimitToImport}`,
        )

        const date = new Date()
        const timestampLimitToImport =
          new Date(date.setDate(date.getDate() - daysLimitToImport)).getTime() /
          1000
    ...
    'messages.upsert': async (
      ...
      try {
        ...
        for (const received of messages) {
          ...
          const retryCache =
            (await this.baileysCache.get(received.key.id)) || null
          ...
          const contentMsg = received?.message[
            getContentType(received.message)
          ] as any
          ...
          if (
            this.localSettings.read_messages &&
          ...
          if (
            this.localSettings.read_status &&
            received.key.id === 'status@broadcast'
          ) {
            await this.client.readMessages([received.key])
          }
          ...
          this.logger.verbose(
            'Sending data to webhook in event MESSAGES_UPSERT',
          )
          ...
          if (
            (this.localTypebot.enabled && type === 'notify') ||
            typebotSessionRemoteJid
          ) {
            if (
              !(
                this.localTypebot.listening_from_me === false &&
          ...
          if (
            this.localChamaai.enabled &&
            messageRaw.key.fromMe === false &&
            type === 'notify'
          ) {
          ...
          const contactRaw: ContactRaw = {
            ...
            pushName: received.pushName,
            profilePictureUrl: (
              await this.profilePicture(received.key.remoteJid)
            ).profilePictureUrl,
            owner: this.instance.name,
          }
          ...
          if (contact?.length) {
            ...
            this.logger.verbose(
              'Sending data to webhook in event CONTACTS_UPDATE',
            )
          ...
          this.logger.verbose(
            'Sending data to webhook in event CONTACTS_UPSERT',
          )
      ...
      for await (const {key, update} of args) {
        ...
        if (key.remoteJid !== 'status@broadcast') {
          ...
          if (update.message === null && update.status === undefined) {
            ...
            this.logger.verbose(
              'Sending data to webhook in event MESSAGE_DELETE',
            )
          ...
          this.logger.verbose(
            'Sending data to webhook in event MESSAGES_UPDATE',
          )
          this.sendDataWebhook(Events.MESSAGES_UPDATE, message)

          this.logger.verbose('Inserting message in database')
          this.repository.messageUpdate.insert(
            [message],
            this.instance.name,
            database.SAVE_DATA.MESSAGE_UPDATE,
          )
  ...
  private readonly groupHandler = {
    ...
    'group-participants.update': (participantsUpdate: {
      ...
      this.logger.verbose(
        'Sending data to webhook in event GROUP_PARTICIPANTS_UPDATE',
      )
        ...
        if (chat) {
          ...
          if (data.type === 'remove') {
            labels = labels.filter(
              (label) => label !== data.association.labelId,
            )
  ...
  private async sendMessageWithTyping<T = proto.IMessage>(
    ...
    try {
      ...
      const contentMsg = messageSent.message[
        getContentType(messageSent.message)
      ] as any
    ...
    try {
      ...
      if (data?.options?.delay && data?.options?.delay > 20000) {
        ...
        while (remainingDelay > 20000) {
          ...
          await this.client.sendPresenceUpdate(
            (data?.options?.presence as WAPresence) ?? 'composing',
            sender,
          )
  ...
  private async formatStatusMessage(status: StatusMessage) {
    ...
    if (status.type === 'audio') {
      ...
      const convert = await this.processAudio(
        status.content,
        'status@broadcast',
      )
  ...
  private async prepareMediaMessage(mediaMessage: MediaMessage) {
    try {
      ...
      if (mediaMessage.mediatype === 'video') {
        this.logger.verbose(
          'Is media type video then set gif playback as false',
        )
  ...
  public async mediaSticker(data: SendStickerDto) {
    ...
    const convert = await this.convertToWebP(
      data.stickerMessage.image,
      data.number,
    )
  ...
  public async whatsappNumber(data: WhatsAppNumberDto) {
    ...
    const contacts: ContactRaw[] = await this.repository.contact.findManyById({
      owner: this.instance.name,
      ids: jids.users.map(({jid}) =>
        jid.startsWith('+') ? jid.substring(1) : jid,
      ),
    })
  ...
  public async updatePrivacySettings(settings: PrivacySettingDto) {
    ...
    try {
      await this.client.updateReadReceiptsPrivacy(
        settings.privacySettings.readreceipts,
      )
      this.logger.verbose('Read receipts privacy updated')

      await this.client.updateProfilePicturePrivacy(
        settings.privacySettings.profile,
      )
      ...
      await this.client.updateGroupsAddPrivacy(
        settings.privacySettings.groupadd,
      )
  ...
  public async acceptInviteCode(id: AcceptGroupInvite) {
    this.logger.verbose(
      'Joining the group by invitation code: ' + id.inviteCode,
    )
    try {
