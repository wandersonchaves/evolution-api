...
export class WAMonitoringService {
  constructor(
    ...
    this.dbInstance = this.db.ENABLED
      ? this.repository.dbServer?.db(
          this.db.CONNECTION.DB_PREFIX_NAME + '-instances',
        )
      : undefined
  }
  ...
  public readonly waInstances: Record<
    string,
    BaileysStartupService | BusinessStartupService
  > = {}
    ...
    if (typeof time === 'number' && time > 0) {
      ...
      setTimeout(async () => {
        if (this.waInstances[instance]?.connectionStatus?.state !== 'open') {
          if (
            this.waInstances[instance]?.connectionStatus?.state === 'connecting'
          ) {
            if (
              (await this.waInstances[instance].findIntegration())
  ...
  public async instanceInfo(instanceName?: string, arrayReturn = false) {
    ...
    for await (const [key, value] of Object.entries(this.waInstances)) {
      if (value) {
        ...
        if (findIntegration) {
          integration = {
            ...findIntegration,
            webhook_wa_business: `${urlServer}/webhook/whatsapp/${encodeURIComponent(
              key,
            )}`,
          }
        }
        ...
        if (value.connectionStatus.state === 'open') {
          ...
          if (
            this.configService.get<Auth>('AUTHENTICATION')
    ...
    if (arrayReturn) {
      return [
        instances.find((i) => i.instance.instanceName === instanceName) ??
          instances,
      ]
    }
    return (
  ...
  private delInstanceFiles() {
    ...
    setInterval(async () => {
      if (this.db.ENABLED && this.db.SAVE_DATA.INSTANCE) {
        ...
        !this.redis.REDIS.ENABLED &&
        !this.redis.REDIS.SAVE_INSTANCES
      ) {
  ...
  public async cleaningStoreFiles(instanceName: string) {
    if (!this.db.ENABLED) {
      ...
      execSync(
        `rm -rf ${join(STORE_DIR, 'auth', 'apikey', instanceName + '.json')}`,
      )
    ...
    try {
      ...
      if (this.db.ENABLED && this.db.SAVE_DATA.INSTANCE) {
        ...
        await this.dbInstance
          .collection(data.instanceName)
          .replaceOne({_id: 'integration'}, msgParsed, {
            upsert: true,
          })
      } else {
  ...
  private async setInstance(name: string) {
    ...
    if (
      integration &&
      integration.integration === Integration.WHATSAPP_BUSINESS
    ) {
    ...
    if (collections.length > 0) {
      ...
      await Promise.all(
        collections.map((coll) =>
          this.setInstance(coll.namespace.replace(/^[\w-]+\./, '')),
        ),
      )
    } else {
  ...
  private async loadInstancesFromFiles() {
    ...
    await Promise.all(
      instanceDirs.map(async (instanceName) => {
        this.logger.verbose(
          'Reading instance files and setting instances: ' + instanceName,
        )
        const files = readdirSync(join(INSTANCE_DIR, instanceName), {
          encoding: 'utf-8',
        })

        if (files.length === 0) {
          rmSync(join(INSTANCE_DIR, instanceName), {
            recursive: true,
            force: true,
          })
        } else {
  ...
  private removeInstance() {
    this.eventEmitter.on('remove.instance', async (instanceName: string) => {
      ...
      try {
        this.logger.verbose(
          'instance: ' + instanceName + ' - removing from memory',
        )
  ...
  private async deleteTempInstances(collections: Collection<Document>[]) {
    ...
    auths.forEach((auth) => {
      if (
        collections.find(
          (coll) => coll.namespace.replace(/^[\w-]+\./, '') === auth._id,
        )
      ) {
        return
      }
